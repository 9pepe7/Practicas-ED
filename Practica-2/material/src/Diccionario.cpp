...
using namespace std;
  ...
  Diccionario::Diccionario(Vector_Dinamico<Termino> v)
  {
    tamanio = v.size();
    dicc = v;
  }
  ...
  ostream& operator<<(ostream & flujo, const Diccionario &d)
  {
    ...
    {
      Termino ter = d.getTermino(i);
      //flujo << ter; Esto no funcionaría
      for(int i=0;i<ter.getTamanio();++i){
        flujo << ter.getPalabra() << ";" << ter.getDefiniciones()[i] << endl;
      }
    }
    return flujo;
  }
